...
class TestCollectNodes:
    def test_collect_nodes_includes_descendants(self):
        from article_extractor.url_normalizer import _collect_nodes

        doc = JustHTML("<div><a></a><a></a></div>")
        root = doc.query("div")[0]
        nodes = _collect_nodes(root, ("a",))
        assert len(nodes) == 2

    def test_collect_nodes_includes_root_if_matching(self):
        from article_extractor.url_normalizer import _collect_nodes

        doc = JustHTML("<a><span></span></a>")
        root = doc.query("a")[0]
        nodes = _collect_nodes(root, ("a",))
        assert len(nodes) == 1
        assert nodes[0] == root

    def test_collect_nodes_excludes_root_if_not_matching(self):
        from article_extractor.url_normalizer import _collect_nodes

        doc = JustHTML("<div><a></a></div>")
        root = doc.query("div")[0]
        nodes = _collect_nodes(root, ("a",))
        assert len(nodes) == 1
        assert nodes[0] != root

    def test_collect_nodes_multiple_tags(self):
        from article_extractor.url_normalizer import _collect_nodes

        doc = JustHTML("<div><img><a></a></div>")
        root = doc.query("div")[0]
        nodes = _collect_nodes(root, ("img", "a"))
        assert len(nodes) == 2
